  ...
  deelnemers.forEach(d => {
    ...
    periodes.forEach(p => {
      ...
      if (p.id === 'namiddag2' && isDubbelGekozenInN1) {
        ...
        options = `<option value="${n1GekozenActId}" selected disabled>⏩ ${dubbeleActObj ? dubbeleActObj.naam : 'Vervolg Namiddag 1'}</option>`;
        ingevuldePeriodes++;
      } else if (!heeftAanbod) {
      ...
      gridHtml += `
        <div class="periode-box ${isVakGeblokkeerd ? 'periode-disabled' : ''}">
          <div class="periode-title">${p.label}</div>
          <select class="form-control" data-deelnemer="${d.id}" data-periode="${p.id}" ${isVakGeblokkeerd ? 'disabled' : ''} onchange="onKeuzeChange('${kampId}')">${options}</select>
        </div>
      `;
    });
    ...
    if (s.value && s.value !== 'unselected') {
      ...
      if (p === 'namiddag1') {
        ...
        if (itemInfo && itemInfo.isDubbel) {
          appData.keuzes[`${dId}_namiddag2`] = s.value;
        }
      }
    } else {
      delete appData.keuzes[`${dId}_${p}`];
      if (p === 'namiddag1') {
        delete appData.keuzes[`${dId}_namiddag2`];
      }
    }
  });
...
// 2. BEHEER: CHECKBOXES AANBOD, LEEFTIJDEN & DUBBELE BLOKKEN
...
window.onBeheerKampSelectionChange = function() {
  ...
  periodes.forEach(p => {
    ...
    box.innerHTML = appData.masterActiviteiten.map(act => {
      ...
      const minLeeftijd = bestaand ? (bestaand.minLeeftijd || 0) : 0;
...
window.opslaanBeheerAanbodMulti = async function() {
  ...
  for (const kampId of geselecteerdeKampIds) {
    ...
    periodes.forEach(p => {
      ...
      cbs.forEach(cb => {
        ...
        const minLInput = document.querySelector(`.minL-beheer-${p}[data-act="${actId}"]`);
        const maxLInput = document.querySelector(`.maxL-beheer-${p}[data-act="${actId}"]`);
        const isDubbelCb = document.querySelector(`.is-dubbel-beheer-${p}[data-act="${actId}"]`);

        const maxVal = parseInt(maxInput ? maxInput.value : 0) || 0;
        const minLVal = parseInt(minLInput ? minLInput.value : 0) || 0;
        const maxLVal = parseInt(maxLInput ? maxLInput.value : 0) || 0;
        const isDubbelVal = isDubbelCb ? isDubbelCb.checked : false;
  ...
  showModal("Opgeslagen", "Het activiteitenaanbod, de leeftijdsgrenzen en instellingen zijn opgeslagen.");
...
window.verwijderDeelnemer = async function(id) {
  ...
  if (bevestig) {
    ...
    renderBeheerLijsten();
    updateStats();
    showModal("Verwijderd", "Deelnemer is verwijderd uit de database.");
  }
};
...
window.exportCSV = function() {
  const geselecteerdKeuzesKamp = document.getElementById('kampSelect')?.value;
  const geselecteerdOverzichtKamp = document.getElementById('overzichtKampSelect')?.value;
  const filterKamp = geselecteerdKeuzesKamp || geselecteerdOverzichtKamp || '';
...
window.openTab = function(tabName) {
  ...
  const btnElem = document.getElementById(`btn-tab-${tabName}`);
